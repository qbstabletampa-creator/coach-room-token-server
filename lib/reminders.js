 ...
 */
function buildRemindersHandler({ notify, rulesProvider, waitlistExpire = null } = {}) {
  return async function remindersHandler(req, res) {
    ...
    try {
      ...
      if (waitlistExpire) {
        try {
          await waitlistExpire({ now: new Date(nowMs), limit: 200 });
        } catch (err) {
          console.error("[reminders:hook:waitlist-expiry] failed (non-fatal)");
        }
      }
