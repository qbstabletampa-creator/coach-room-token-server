...
async function fetchBookedSlots(s, { loIso, hiIso }) {
  const q =
 ...
 */
function buildRemindersHandler({ notify } = {}) {
  return async function remindersHandler(req, res) {
    ...
    try {
      ...
      const slots = await fetchBookedSlots(s, { loIso, hiIso });

      let scanned = 0;
      let sent = 0;
      for (const slot of Array.isArray(slots) ? slots : []) {
        scanned++;
        const w = windowForSlot(new Date(slot.starts_at).getTime(), nowMs);
        if (!w) continue;
        ...
        for (const r of resolveRecipients(slot)) {
          ...
          try {
            ...
            console.error("[reminders] notify failed:", r.userId, slot.id, err);
            continue;
          }
