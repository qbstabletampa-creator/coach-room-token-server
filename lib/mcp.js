...
function buildMcpServer({ apiCore, coachId }) {
  ...
  reg(
    "generate_slots",
    {
      ...
      description:
        "Expand your active availability windows into concrete open slots over a horizon. Pass days (1-84, primary) or the legacy weeks (1-12); default 28 days. Skips blackout dates. Idempotent: re-running skips slots that already exist.",
      inputSchema: {
        days: z.number().int().min(1).max(84).optional(),
        weeks: z.number().int().min(1).max(12).optional(),
      },
    },
    (a) => apiCore.generateSlots({ coachId, weeks: a.weeks, days: a.days }),
  );
  ...
  // ---- app config (nomenclature, saved views, pins, layout) ----
  // These let an AI agent configure the app for the coach: rename labels, save
  // views, set the pinned shortcut row. Every tool is a one-liner over apiCore
  // with the server-derived coachId; the tenant lock lives in the core.
  const nsEnum = z.enum(["labels", "pins", "views", "layout"]);
  reg("get_config", { title: "Get config", description: "Read the coach's app config. Omit namespace for the full snapshot, or pass one of labels/pins/views/layout for just that namespace.", inputSchema: { namespace: nsEnum.optional() } },
    (a) => apiCore.getConfig({ coachId, namespace: a.namespace }));
  reg("set_config_key", { title: "Set a config key", description: "Upsert one config key. namespace is labels/pins/views/layout, key is a stable id, value is any JSON <= 16 KB. Atomic per key.", inputSchema: { namespace: nsEnum, key: z.string(), value: z.unknown() } },
    (a) => apiCore.setConfig({ coachId, namespace: a.namespace, key: a.key, value: a.value }));
  reg("delete_config_key", { title: "Delete a config key", description: "Delete one config key in a namespace. Idempotent.", inputSchema: { namespace: nsEnum, key: z.string() } },
    (a) => apiCore.deleteConfig({ coachId, namespace: a.namespace, key: a.key }));
  reg("rename_label", { title: "Rename a label", description: "Rename a user-facing label (button/column/tab) by its stable label id. Overrides the default wherever that id renders.", inputSchema: { label_id: z.string(), text: z.string() } },
    (a) => apiCore.renameLabel({ coachId, labelId: a.label_id, text: a.text }));
  reg("list_saved_views", { title: "List saved views", description: "List the coach's saved views for one screen (filters, sort, column visibility).", inputSchema: { screen: z.string() } },
    (a) => apiCore.listSavedViews({ coachId, screen: a.screen }));
  reg("save_view", { title: "Save a view", description: "Save (or overwrite) a named view for a screen. view = { id, name, screen, filters, sort, columns? }.", inputSchema: { view: z.object({ id: z.string(), name: z.string(), screen: z.string(), filters: z.record(z.string(), z.unknown()), sort: z.string().nullable(), columns: z.record(z.string(), z.boolean()).optional() }) } },
    (a) => apiCore.saveView({ coachId, view: a.view }));
  reg("delete_view", { title: "Delete a saved view", description: "Delete one saved view by screen + view id.", inputSchema: { screen: z.string(), view_id: z.string() } },
    (a) => apiCore.deleteView({ coachId, screen: a.screen, viewId: a.view_id }));
  reg("get_pins", { title: "Get pinned shortcuts", description: "Read the coach's pinned Menu shortcut row (ordered).", inputSchema: {} },
    () => apiCore.getPins({ coachId }));
  reg("set_pins", { title: "Set pinned shortcuts", description: "Replace the whole pinned Menu shortcut row in one atomic write. items = [{ id, label, route, icon }] in display order.", inputSchema: { items: z.array(z.object({ id: z.string(), label: z.string(), route: z.string(), icon: z.string() })) } },
    (a) => apiCore.setPins({ coachId, items: a.items }));
