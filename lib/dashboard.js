...
async function getDashboardData({ coachId, range = "this_month", now = new Date() }) {
  if (!sb()) return { status: 503, data: { error: "dashboard not configured" } };
  const label = String(range || "this_month");
  ...
  const [curRev, prevRev, bookings, openSlots, needs, sources] = await Promise.all([
    ...
    revenueByRail({ coachId, from: prevFrom, to: from }),
  ...
  return { status: 200, data: {
    ...
    revenue: { current_cents, previous_cents,
      delta_pct: deltaPct(current_cents, previous_cents), by_rail: curRev.by_rail || {} },
    bookings, open_slots: openSlots, needs_attention: needs, source_attribution: sources,
  } };
}
  ...
  async function getDashboard(req, res) {
    try {
      ...
      const result = await getDashboardData({
        coachId, range: req.query && req.query.range,
      });
