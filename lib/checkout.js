...
async function getOrCreateStripeCustomer(s, stripe, { coachId, athleteId, email }) {
  const query =
  ...
  const customer = await stripe.customers.create(
    ...
    { idempotencyKey: `stripe-customer:${coachId}:${athleteId}` },
  );
  ...
  return async function createSession(req, res) {
    try {
      ...
      try {
        ...
        const customer = await getOrCreateStripeCustomer(s, stripe, {
          coachId: pkg.coach_id,
