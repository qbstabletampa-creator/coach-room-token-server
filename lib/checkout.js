...
// (a platform customer id is not usable on a connected account). Omitted =>
// platform account, unchanged behavior. The stored stripe_customers mapping is
// keyed by (coach_id, athlete_id); since a coach's connected account is 1:1 with
// coach_id, a stored id is naturally scoped to that coach's account context.
async function getOrCreateStripeCustomer(s, stripe, { coachId, athleteId, email }, requestOpts = {}) {
  const query =
    `${s.url}/rest/v1/stripe_customers?coach_id=eq.${encodeURIComponent(coachId)}` +
  ...
  const customer = await stripe.customers.create(
    ...
    { idempotencyKey: `stripe-customer:${coachId}:${athleteId}`, ...requestOpts },
  );
  const insert = await fetch(`${s.url}/rest/v1/stripe_customers`, {
    method: "POST",
