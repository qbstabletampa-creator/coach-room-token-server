...
//
// DARK BEHIND A FLAG: everything here is inert until the connect flag is "1"/"true".
// The flag check is exported (connectEnabled) so index.js can gate route mounting;
// checkout.js and the webhook also consult it so a stray connected event or a
// coach row can never change money routing while the flag is off. The flag name is
// PAYMENTS_CONNECT_ENABLED (spec) with CONNECT_ENABLED honored as an established
// alias — see connectEnabled() and the PM note.
//
// CONTRACT coded against (migration 0033, owned by the other agent — its comment
// lists exactly these three: "pending / active / restricted"):
//   coaches.stripe_account_id text   — the connected account id (acct_...)
//   coaches.connect_status     text  — one of: 'pending' | 'active' | 'restricted'
//                                       'pending'    = created / onboarding unfinished /
//                                                      submitted-and-under-review (no charges yet)
//                                       'active'     = charges_enabled === true (eligible to collect)
//                                       'restricted' = Stripe requirements.disabled_reason is set
...
// The dark-launch flag. ON only for the exact strings "1" or "true" (identical
// semantics to index.js's envFlag). Read at CALL time so tests/env flips apply.
