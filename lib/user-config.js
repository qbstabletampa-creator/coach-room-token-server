...
// ---- named saved-views bucket (mirrors the app codec: lib/saved-views.ts) ----
// The CoachTime app persists ALL explicit named views for a screen under ONE
// reserved key `<screen>:__named` in the views namespace, value `{ views:
// SavedView[] }` — NOT one key per view — because the client reads config through
// a per-key reactive hook (useConfig) and React forbids a hook-per-view for a
// dynamic count. The implicit last-used state is a SEPARATE key `<screen>:__last`
// and is never touched here. SavedView shape (codec `toSavedView`):
//   { id, name, screen, filters: Record, sort: string|null, columns?: Record }
// A view failing this guard is DROPPED by the app on read (its isSavedView
// filter), so the server refuses to write it — an agent save must land in a shape
// the phone actually surfaces.
const NAMED_VIEWS_ID = "__named";
const LAST_VIEW_ID = "__last";
const RESERVED_VIEW_IDS = new Set([NAMED_VIEWS_ID, LAST_VIEW_ID]);
// Conservative bucket ceiling: the 0031 DDL caps jsonb at 16 KB and the app
// silently reverts a write at the cap. We gate on the serialized bucket size with
// headroom below 16 KB so an agent write never corrupts (silently drops) the bucket.
const NAMED_BUCKET_MAX_BYTES = 15 * 1024;
function namedKey(screen) { return `${screen}:${NAMED_VIEWS_ID}`; }
function isReservedViewId(id) { return RESERVED_VIEW_IDS.has(id); }
// Structural guard identical to the app's isSavedView (lib/saved-views.ts). Kept
// in lockstep: anything false here is dropped by the phone, so we never store it.
function isSavedViewShape(v) {
  return ownObject(v) && typeof v.id === "string" && v.id.length > 0 &&
    typeof v.name === "string" && typeof v.screen === "string" &&
    ownObject(v.filters) && (v.sort === null || typeof v.sort === "string");
}
// Read the named-view list out of a stored bucket, fail-soft to [] — mirrors the
// app's readNamedViews: drop non-SavedView elements and any reserved id.
function readNamedViews(bucket) {
  const items = ownObject(bucket) && Array.isArray(bucket.views) ? bucket.views : [];
  return items.filter((it) => isSavedViewShape(it) && !isReservedViewId(it.id));
}
// Canonicalize to exactly the fields the app's ViewCodec emits (drop extras).
function canonicalView(view) {
  const out = { id: view.id, name: view.name, screen: view.screen, filters: view.filters, sort: view.sort === undefined ? null : view.sort };
  if (ownObject(view.columns)) out.columns = view.columns;
  return out;
}
// Read ONLY the `<screen>:__named` row (proves __last is never in scope).
async function readNamedBucket(s, coachId, screen) {
  const rows = await db(s, `${tableFilter(coachId, `&namespace=eq.views&key=eq.${enc(namedKey(screen))}`)}&select=value`);
  if (!Array.isArray(rows)) throw new DbError("GET", 200, "invalid representation");
...
async function listSavedViews({ coachId, screen }) {
  ...
  const s = service(); if (!s) return notConfigured();
  const views = readNamedViews(await readNamedBucket(s, coachId, screen));
  return { status: 200, data: { screen, views } };
}
async function saveView({ coachId, view }) {
  if (!ownObject(view)) return bad("invalid_request");
  const { screen, id } = view;
  if (typeof screen !== "string" || !KEY_RE.test(screen)) return bad("invalid_request");
  if (typeof id !== "string" || !KEY_RE.test(id) || isReservedViewId(id)) return bad("invalid_request");
  const candidate = canonicalView(view);
  if (!isSavedViewShape(candidate)) return bad("invalid_request"); // would be dropped by the app on read
  const s = service(); if (!s) return notConfigured();
  const current = readNamedViews(await readNamedBucket(s, coachId, screen));
  const at = current.findIndex((v) => v.id === id);
  const merged = at === -1 ? [...current, candidate] : current.map((v) => (v.id === id ? candidate : v));
  const payload = { views: merged };
  if (Buffer.byteLength(JSON.stringify(payload), "utf8") > NAMED_BUCKET_MAX_BYTES) {
    return { status: 413, data: { error: "view_bucket_too_large" } };
  }
  const wrote = await setConfig({ coachId, namespace: "views", key: namedKey(screen), value: payload });
  if (wrote.status !== 200) return wrote;
  return { status: 200, data: { screen, view: candidate } };
}
async function deleteView({ coachId, screen, viewId }) {
  if (typeof screen !== "string" || !KEY_RE.test(screen)) return bad("invalid_request");
  if (typeof viewId !== "string" || !KEY_RE.test(viewId) || isReservedViewId(viewId)) return bad("invalid_request");
  const s = service(); if (!s) return notConfigured();
  const current = readNamedViews(await readNamedBucket(s, coachId, screen));
  if (!current.some((v) => v.id === viewId)) return { status: 200, data: { deleted: false, screen, viewId } };
  const wrote = await setConfig({ coachId, namespace: "views", key: namedKey(screen), value: { views: current.filter((v) => v.id !== viewId) } });
  if (wrote.status !== 200) return wrote;
  return { status: 200, data: { deleted: true, screen, viewId } };
}
...
module.exports = {
  ...
  // named saved-views bucket helpers (mirror app lib/saved-views.ts)
  NAMED_VIEWS_ID, LAST_VIEW_ID, namedKey, readNamedViews, isSavedViewShape, NAMED_BUCKET_MAX_BYTES,
};
